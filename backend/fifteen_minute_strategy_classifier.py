...
This stage is intentionally additive. It consumes the canonical hourly Top-20,
verifies the latest fully closed 15-minute candle, and reuses the already
installed strategy engine, setup-direction helpers, and deterministic grading
thresholds. The engine is evaluated with interval ``5`` so its ``5M`` input is
real closed 5-minute market data rather than a 15-minute alias.

Step 5 never creates an execution candidate, never writes to the confirmed
entry queue, and never submits an order. A later 5M entry-confirmation stage is
responsible for turning a classified setup into an executable command.
"""
...
def _target_candle_open_seconds(timestamp: int) -> int:
    """Return the opening time of the latest fully closed UTC-aligned 15M bar."""
...
def _snapshot_unlocked(status_override: str | None = None) -> dict[str, Any]:
    return {
        ...
        "source": str(
            _STATE.get("source")
            or "hourly_top20_closed_15m_strategy_classification"
        ),
...
def _persist(payload: dict[str, Any]) -> bool:
    ...
    body = {
        ...
        "fifteenMinuteCandleTime": payload["fifteenMinuteCandleTime"],
    ...
    return bool(
        ...
        and confirmed.get("fifteenMinuteCandleTime")
        == body["fifteenMinuteCandleTime"]
        and list(confirmed.get("symbols") or []) == body["symbols"]
    )


def _hourly_snapshot(core: Any) -> dict[str, Any]:
    reader = getattr(core, "hourly_watchlist", None)
    if callable(reader):
        payload = reader(False)
        if isinstance(payload, dict):
            return dict(payload)
    reader = getattr(core, "hourly_watchlist_status", None)
    if callable(reader):
        payload = reader()
        if isinstance(payload, dict):
            return dict(payload)
...
def _vote_payload(vote: dict[str, Any], expected_side: str) -> dict[str, Any]:
    ...
    return {
        **item,
        "alignedWithWatchlist": item.get("signal") == expected_side,
        "grade": grading["grade"],
        "gradeScore": grading["gradeScore"],
        "gradeExecutionEligible": grading["executionEligible"],
        "watchOnly": grading["watchOnly"],
    }
...
def _classify_symbol(
    ...
    trend = str(
        watchlist_row.get("oneHourTrend")
        or watchlist_row.get("trend")
        or ""
    ).upper()
    base = {
        "symbol": symbol,
    ...
    if not symbol or _EXPECTED_SIDE is None or _ACTIONABLE_VOTE is None:
        return {
            **base,
            "status": "ERROR",
            "reason": "Strategy classifier dependencies are unavailable",
        }, "dependencyError"
    ...
    if expected_side is None:
        return {
            **base,
            "status": "NO_SETUP",
            "reason": "Hourly watchlist direction is unsupported",
    ...
    if not history:
        return {
            **base,
            ...
            "status": "NO_SETUP",
            "reason": "Not enough fully closed 15M candle history",
        }, "missing15mHistory"
    ...
    if latest_15m != target_open_ms:
        return {
            **base,
            "expectedSide": expected_side,
            "observedFifteenMinuteCandleTime": latest_15m,
            "status": "NO_SETUP",
            "reason": "Latest fully closed 15M candle is stale",
        }, "stale15mCandle"
    ...
    try:
        ...
        return {
            **base,
            "expectedSide": expected_side,
            "status": "ERROR",
            "reason": f"Strategy engine failed: {exc}",
        }, "engineError"
    ...
    if entry_interval != _ENTRY_INTERVAL or five_minute_candle_time is None:
        return {
            **base,
            ...
            "reason": "Real closed 5M strategy context is unavailable",
        }, "invalid5mContext"
    ...
    aligned_engines = [
        str(vote.get("engine") or "")
        for vote in raw_votes
        if vote.get("signal") == expected_side
    ]
    opposing_side = "Sell" if expected_side == "Buy" else "Buy"
    opposing_engines = [
        str(vote.get("engine") or "")
        for vote in raw_votes
        if vote.get("signal") == opposing_side
    ]
    waiting_engines = [
        str(vote.get("engine") or "")
        for vote in raw_votes
        if vote.get("signal") == "WAIT"
    ]
    ...
    if aligned_vote is not None:
        ...
        return {
            **common,
            ...
            "gradeExecutionEligible": grading["executionEligible"],
            "watchOnly": grading["watchOnly"],
            "reason": "Trend-aligned strategy setup classified; awaiting Step 6 closed-5M entry confirmation",
        }, None
    ...
    if waiting_engines:
        return {
            **common,
            "status": "WATCHING",
            "strategy": None,
            "reason": "No trend-aligned actionable vote; existing strategies remain in WAIT state",
        }, None
    ...
    return {
        **common,
        "status": "NO_SETUP",
        "strategy": None,
        "reason": "No trend-aligned strategy setup on the closed 15M candle",
    }, None


def build(core: Any, now: int | None = None) -> dict[str, Any]:
    """Classify the hourly Top-20 once for the latest fully closed 15M candle."""
    ...
    try:
        ...
        for watchlist_row in upstream_rows[:20]:
            row, rejection = _classify_symbol(
                core,
                watchlist_row,
                target_open_ms,
                now_ms,
        ...
        metrics = {
            ...
            "processed": len(rows),
            "setupClassified": sum(
                1 for row in rows if row.get("status") == "SETUP_CLASSIFIED"
            ),
        ...
        payload = {
            ...
            "rows": rows,
            "metrics": metrics,
            "lastError": None if rows else "No watchlist symbol was classified",
            "persisted": False,
        }
...
def install(core: Any, setup_worker: Any) -> dict[str, Any]:
    """Install callbacks without replacing the legacy setup/execution path."""
    ...
    core.fifteen_minute_strategy_classification = (
...
def status(core: Any | None = None) -> dict[str, Any]:
    return {
        "installed": bool(
            core is not None
            and getattr(core, "_fifteen_minute_strategy_classifier_v1_installed", False)
        ),
        "policy": "EVERY_CLOSED_15M_CLASSIFY_EXISTING_STRATEGIES",
        "usesRealFiveMinuteContext": True,
        "createsEntryCandidate": False,
        "writesConfirmedQueue": False,
        "submitsOrder": False,
        "snapshot": snapshot(),
    }
