...
const EXECUTION_OWNED = ['RESERVED', 'ORDER_PENDING'];
...
export class ExecutionRepository {
  ...
  async ping() {
    const result = await this.pool.query(
      "SELECT COALESCE(MAX(version),0)::int AS version, to_regclass('public.execution_commands') AS table_name FROM schema_migrations",
    );
  ...
  async claim(ownerId, slotId) {
    ...
    try {
      ...
      const occupied = await client.query(
        'SELECT 1 FROM execution_commands WHERE slot_id=$1 AND state = ANY($2::text[]) LIMIT 1',
        [slotId, ACTIVE],
      );
      if (occupied.rowCount) { await client.query('COMMIT'); return null; }
      const active = await client.query(
        'SELECT COUNT(*)::int AS count FROM execution_commands WHERE state = ANY($1::text[])',
        [ACTIVE],
      );
      if (Number(active.rows[0].count) >= 3) { await client.query('COMMIT'); return null; }
      const selected = await client.query(
        "SELECT candidate_key FROM execution_commands WHERE state='AVAILABLE' ORDER BY created_at,candidate_key FOR UPDATE SKIP LOCKED LIMIT 1",
      );
      if (!selected.rowCount) { await client.query('COMMIT'); return null; }
      const result = await client.query(
        "UPDATE execution_commands SET state='RESERVED',slot_id=$1,owner_id=$2,updated_at=$3 WHERE candidate_key=$4 AND state='AVAILABLE' RETURNING *",
        [slotId, ownerId, nowSeconds(), selected.rows[0].candidate_key],
      );
  ...
  async ownedExecutionCommand(ownerId, slotId) {
    const result = await this.pool.query(
      'SELECT * FROM execution_commands WHERE owner_id=$1 AND slot_id=$2 AND state = ANY($3::text[]) ORDER BY updated_at LIMIT 1',
      [ownerId, slotId, EXECUTION_OWNED],
    );
  ...
  async transition(command, nextState) {
    ...
    const result = await this.pool.query(
      'UPDATE execution_commands SET state=$1,updated_at=$2 WHERE candidate_key=$3 AND owner_id=$4 AND state=$5 RETURNING *',
      [nextState, nowSeconds(), command.candidateKey, command.ownerId, expected],
    );
  ...
  async pendingReservedMargin(candidateKey) {
    const result = await this.pool.query(
      `SELECT COALESCE(SUM(
        CASE WHEN (payload->>'requiredInitialMarginUsdt') ~ '^[0-9]+(\\.[0-9]+)?$'
        THEN (payload->>'requiredInitialMarginUsdt')::numeric ELSE 0 END
      ),0)::text AS amount
      FROM execution_commands
      WHERE candidate_key<>$1 AND state = ANY($2::text[])`,
      [candidateKey, ['RESERVED', 'ORDER_PENDING']],
    );
  ...
  async recordOrder(command, status, evidence = {}) {
    ...
    await this.pool.query(
      `INSERT INTO orders(order_key,symbol,side,status,payload,updated_at)
       VALUES($1,$2,$3,$4,$5::jsonb,$6)
       ON CONFLICT(order_key) DO UPDATE SET
         symbol=EXCLUDED.symbol,side=EXCLUDED.side,status=EXCLUDED.status,
         payload=EXCLUDED.payload,updated_at=EXCLUDED.updated_at`,
      [command.candidateKey, command.payload?.symbol, command.payload?.side, status, JSON.stringify(payload), nowSeconds()],
    );
  }
  ...
  async recordFills(command, executions = []) {
    for (const execution of executions) {
      ...
      await this.pool.query(
        `INSERT INTO fills(fill_key,order_key,symbol,qty,price,payload,created_at)
         VALUES($1,$2,$3,$4,$5,$6::jsonb,$7)
         ON CONFLICT(fill_key) DO NOTHING`,
        [fillKey, command.candidateKey, command.payload?.symbol, execution.execQty, execution.execPrice,
          JSON.stringify({ ...execution, candidateKey: command.candidateKey }),
          Math.floor(Number(execution.execTime || Date.now()) / 1000)],
      );
    }
  }
}
...
function normalize(row) {
  ...
  return {
    ...
    slotId: row.slot_id === null || row.slot_id === undefined ? null : Number(row.slot_id),
    state: row.state,
    payload: row.payload,
